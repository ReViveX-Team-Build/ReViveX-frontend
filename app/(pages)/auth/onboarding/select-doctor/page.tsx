...
import {
  Stethoscope, Search, UserCheck, ArrowRight,
  Loader2, ChevronRight, Star, X, CheckCircle2,
  UserCircle2,
} from "lucide-react";
...
const PAGE_CSS = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .doctor-card.selected {
    border-color: #2DD4BF;
    box-shadow: 0 4px 28px rgba(45,212,191,0.20);
  ...
  .doctor-card.selected .select-badge {
    background: #2DD4BF;
    border-color: #2DD4BF;
  }
...
// TYPES
// ─────────────────────────────────────────────────────────────────────────────
type DoctorListing = Pick<DoctorData, "uid" | "name" | "specialization" | "profilePictureUrl" | "doctorId">;
...
export default function OnboardingSelectDoctor() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      // If patient already has an accepted connection → skip to home
      if (patientData?.connectionStatus === "accepted") {
        router.replace("/patients/home");
        return;
      }
      // If already pending → skip to waiting
  ...
  const handleConfirm = useCallback(async () => {
    ...
    try {
      // 1. Update patient doc: assignedDoctorId + connectionStatus
      await updateDoc(doc(db, "users", user.uid), {
        assignedDoctorId: selected.uid,
        connectionStatus: "pending",
      });

      // 2. Send connection_request communication to the doctor
      ...
      {showModal && selected && (
        <>
          ...
          <div className="modal-card">
            <div style={{ textAlign: "center", marginBottom: 24 }}>
              ...
              <div style={{
                ...
                {selected.profilePictureUrl ? (
                  <img src={selected.profilePictureUrl} alt={selected.name}
                    style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                ) : (
                  <span style={{ fontSize: 22, fontWeight: 800, color: "#2DD4BF" }}>
                    {initials(selected.name)}
                  </span>
                )}
              </div>

              <h3 style={{ fontSize: 17, fontWeight: 800, color: "#0B1E33", marginBottom: 4 }}>
                Send a request to
              </h3>
              <p style={{ fontSize: 18, fontWeight: 700, color: "#2DD4BF" }}>
                {selected.name}
              </p>
            ...
            <p style={{ fontSize: 13.5, color: "#64748b", lineHeight: 1.65, textAlign: "center", marginBottom: 24 }}>
              This doctor will be notified. Once they accept your request, you'll gain access to your therapy portal.
            </p>
            ...
            {error && (
              <div style={{
                ...
                fontSize: 12.5, display: "flex", alignItems: "center", gap: 8, marginBottom: 16,
              }}>
                <X size={14} /> {error}
              </div>
            )}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              >
                {submitting ? (
                  <><Loader2 size={17} style={{ animation: "spin 1s linear infinite" }} /> Sending request…</>
                ) : (
                  <><div className="shimmer" /><UserCheck size={17} /> Send Request</>
                )}
              </button>
              <button
