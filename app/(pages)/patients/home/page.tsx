...
import { Home, Gamepad2, BarChart3, Calendar, Bot, Play } from "lucide-react";
...
export default function PatientDashboard() {
  ...
  const [todaySession, setTodaySession] = useState<ScheduledSession | null>(
    null,
  );
  ...
  useEffect(() => {
    const loadTodaySession = async () => {
      ...
      try {
        const sessions = await getPatientSchedule(user.uid);
        const now = new Date();
        const todayIso = [
          now.getFullYear(),
        ...
        const todaysUpcoming = sessions
          .filter(
            (s) =>
              s.status === "scheduled" &&
              s.scheduledDate === todayIso &&
              !Number.isNaN(
                new Date(`${s.scheduledDate}T${s.scheduledTime}`).getTime(),
              ),
          )
          .sort((a, b) => a.scheduledTime.localeCompare(b.scheduledTime));

        setTodaySession(todaysUpcoming[0] ?? null);
      } catch (e) {
  ...
  const todaySessionTitle = useMemo(() => {
    if (!todaySession) return "No session scheduled today";
    return `${gameLabels[todaySession.gameId]}: Level ${todaySession.level}`;
  }, [todaySession]);
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <main className="flex-1 p-8">
            ...
            <div>
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="text-gray-300 text-sm font-medium">
                  {todaySession
                    ? `Today, ${todaySession.scheduledTime}`
                    : "No upcoming slot"}
                </span>
              </div>

              <h3 className="text-3xl font-bold mb-2">{todaySessionTitle}</h3>
              <p className="text-gray-400 max-w-md">
                {todaySession
                  ? `Duration ${todaySession.durationMinutes} mins • Scheduled ${todaySession.scheduledTime}`
                  : "Visit your schedule to book or review upcoming therapy sessions."}
              </p>
            </div>

            {todaySession?.id ? (
              <Link
                href={`/games/${todaySession.gameId}?sessionId=${todaySession.id}&gameId=${todaySession.gameId}`}>
              ...
              <Link href="/patients/schedule">
                <button className="bg-white/10 hover:bg-white/15 text-white font-bold py-4 px-8 rounded-2xl border border-white/20 transition-all active:scale-95 flex items-center gap-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-sm">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="font-semibold text-teal-600">
                    {mockAdherenceData.nextSession.hand}
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 h-48">
          ...
          <div
            className="h-full flex items-center justify-center text-gray-400 bg-gray-50 
