        ...
        <div
          style={{
            ...
            background: isGame
              ...
              : "rgba(99,102,241,.10)",
            border: isGame
              ? "1.5px solid rgba(45,212,191,.22)"
              : "1.5px solid rgba(99,102,241,.22)",
            color: isGame ? "#2DD4BF" : "#6366f1",
          }}>
          {isGame ? (
            gameEmoji[item.gameId as GameId]
          ) : item.mode === "telehealth" ? (
            <Video size={20} />
          ) : (
            <MapPin size={20} />
          )}
        ...
        <div style={{ minWidth: 0 }}>
          ...
          <div
            ...
            <span
              ...
              {isGame
                ? `${gameLabels[item.gameId as GameId]} · Lv.${item.level}`
                : item.title}
            </span>
            <span style={{ fontSize: 11, color: "#94a3b8" }}>•</span>
            <span className="mono" style={{ fontSize: 11, color: "#2DD4BF" }}>
              {item.scheduledTime}
            </span>
            <span style={{ fontSize: 11, color: "#94a3b8" }}>•</span>
                ...
                style={{
                  ...
                  background:
                    item.mode === "telehealth"
                      ? "rgba(99,102,241,.1)"
                      : "rgba(45,212,191,.1)",
                  color: item.mode === "telehealth" ? "#6366f1" : "#0f766e",
                  fontWeight: 700,
                }}>
      ...
      <div
        ...
        {(item.status === "scheduled" ||
          ...
          <button
            ...
            onClick={() => onCancel(item.id, item.type)}
            disabled={disabled}>
            ...
            {hasSessions && (
              <div
                style={{
                  ...
                  height: 5,
                  borderRadius: "50%",
                  background: isSelected ? "#2DD4BF" : "#6366f1",
                  margin: "4px auto 0",
                  boxShadow: isSelected ? "0 0 5px #2DD4BF" : "none",
                }}
              />
            )}
          </button>
...
export default function SchedulePage() {
  ...
  const [sessionForm, setSessionForm] =
    useState<NewSessionForm>(initialSessionForm);
  const [meetingForm, setMeetingForm] =
    useState<NewMeetingForm>(initialMeetingForm);
  ...
  const refreshData = async (uid: string) => {
    ...
    setError(null);
    try {
      await markMissedSessionsForDoctor(uid).catch((e) =>
        console.warn("Backend mark missed failed:", e),
      );
      await checkUpcomingRemindersForDoctor(uid).catch((e) =>
  ...
  const handleCreateMeeting = async () => {
    if (!user) return;
    if (
      !meetingForm.patientId ||
      !meetingForm.scheduledDate ||
      !meetingForm.scheduledTime
    ) {
      setError("Patient, date, and time are required.");
      return;
    }
    ...
    try {
      ...
      await createAppointment({
        doctorId: user.uid,
        patientId: meetingForm.patientId,
        patientName: pData?.name || "Unknown",
        patientCode: pData?.patientId || "P000",
  ...
  const handleCancel = async (id: string, type: string) => {
    ...
    try {
      ...
      console.error(e);
      setError("Could not cancel item.");
    } finally {
      setActioningId(null);
    }
  };
  ...
  const handleSaveReminderPreference = () => {
    ...
    alert(
      `Preferences saved! Patients will now be notified ${reminderMinutes} minutes prior.`,
    );
  };
      ...
      <div
        ...
        <div
          style={{
            ...
            height: 600,
            background:
              "radial-gradient(circle,rgba(45,212,191,.05),transparent 65%)",
            borderRadius: "50%",
          }}
        />
        <div
          style={{
            position: "absolute",
            inset: 0,
            backgroundImage: `linear-gradient(${isDark ? "rgba(51,65,85,.3)" : "rgba(11,30,51,.02)"} 1px,transparent 1px),linear-gradient(90deg,${isDark ? "rgba(51,65,85,.3)" : "rgba(11,30,51,.02)"} 1px,transparent 1px)`,
      ...
      <div
        ...
        <div
          ...
          <div>
            ...
            <p
              ...
              Manage patient games, telehealth, and clinic visits.
            </p>
          </div>
          <div style={{ display: "flex", gap: 10 }}>
            <button
              className="sc-btn-ghost"
              onClick={() => user && refreshData(user.uid)}
              disabled={loading}>
              <RefreshCw
                size={13}
                style={{
                  animation: loading ? "scSpin 1s linear infinite" : "none",
                }}
              />{" "}
              Refresh
            </button>
        ...
        <div
          style={{
            ...
            marginBottom: 22,
            flexWrap: "wrap",
            animation: "scFadeUp .5s ease .04s both",
          }}>
          {[
            {
              label: "Upcoming",
        ...
        <div
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
            ...
            <div
              className="sc-card"
              style={{
                animation: "scCardPop .5s cubic-bezier(.22,1,.36,1) .06s both",
              }}>
            ...
            <div
              className="sc-card"
              style={{
                animation: "scCardPop .5s cubic-bezier(.22,1,.36,1) .10s both",
              }}>
              <div className="sc-section-title">
                <span
                  className="iw"
                ...
                <span
                  ...
                  {selectedDateAgenda.length} event
                  {selectedDateAgenda.length !== 1 ? "s" : ""}
                </span>
              </div>
              {loading ? (
                <div
                ...
                <div style={{ textAlign: "center", padding: "28px 0" }}>
                  ...
                  <p style={{ fontSize: 12, color: "#94a3b8" }}>
                    Click "New Event" to add a game or meeting.
                  </p>
                </div>
              ) : (
                <div
                  ...
                  {selectedDateAgenda.map((item, i) => (
                    <AgendaRow
                      ...
                      item={item}
                      patientName={
                        patientById.get(item.patientId)?.name ?? item.patientId
                      }
          ...
          {showForm && (
            <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
              <div
                ...
                style={{
                  animation:
                    "scCardPop .5s cubic-bezier(.22,1,.36,1) .08s both",
                  padding: 0,
                  overflow: "hidden",
                }}>
                {/* Tabs */}
                <div
                  ...
                  <div
                    className={`sc-tab ${formType === "meeting" ? "active" : ""}`}
                    onClick={() => setFormType("meeting")}>
                    🤝 Doctor Meeting
                  </div>
                  <div
                    className={`sc-tab ${formType === "game" ? "active" : ""}`}
                ...
                <div style={{ padding: 28 }}>
                  ...
                  <div
                          ...
                          <div style={{ display: "flex", gap: 10 }}>
                            <button
                              ...
                              style={{
                                ...
                                padding: "10px",
                                borderRadius: 12,
                                border: `1.5px solid ${meetingForm.mode === "telehealth" ? "#6366f1" : isDark ? "#475569" : "#e2e8f0"}`,
                                background:
                                  meetingForm.mode === "telehealth"
                                    ? "rgba(99,102,241,0.1)"
                                    : "transparent",
                                color:
                                  meetingForm.mode === "telehealth"
                            ...
                            <button
                              ...
                              style={{
                                ...
                                padding: "10px",
                                borderRadius: 12,
                                border: `1.5px solid ${meetingForm.mode === "in-person" ? "#0f766e" : isDark ? "#475569" : "#e2e8f0"}`,
                                background:
                    ...
                    <button
                      ...
                      disabled={
                        submitting || loading || !user || patients.length === 0
                      }
                        ...
                        <>
                          <div
                            style={{
                              width: 14,
                              height: 14,
                              border: "2.5px solid rgba(11,30,51,.25)",
                              borderTopColor: "#0B1E33",
                              borderRadius: "50%",
              ...
              <div
                ...
                <p
                  ...
                  Patients receive notifications automatically{" "}
                  {reminderMinutes >= 60
                    ? reminderMinutes === 1440
                      ? "24 hours"
                      : "1 hour"
                    : `${reminderMinutes} minutes`}{" "}
                  before scheduled meetings and game sessions.
                </p>
                ...
                {!showReminderConfig ? (
                  ...
                  <div
                    style={{
                      ...
                      animation: "scFadeUp 0.3s ease",
                    }}>
                    <div className="sc-label">
                      Notify patients before event:
                    </div>
                    <div
