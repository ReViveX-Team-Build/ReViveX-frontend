...
import { Search, Bot, Shield, MessageCircle, ClipboardList, User, Loader2, RefreshCw } from 'lucide-react';
...
const MOCK_PATIENTS: DisplayPatient[] = [
  { id:"mock_p1",  name:"P.B. De Silva",        pid:"P001", adherence:45, lastSession:"2025-03-06", status:"Low",    sub:"Standard",     condition:"Stroke",       hasProtocol:true  },
  { id:"mock_p2",  name:"Anura Dissanayaka",    pid:"P002", adherence:92, lastSession:"2025-03-08", status:"High",   sub:"AI Companion", condition:"TBI",          hasProtocol:true  },
  { id:"mock_p3",  name:"Sarath Watawala",      pid:"P003", adherence:78, lastSession:"2025-03-08", status:"High",   sub:"Standard",     condition:"Post-Surgery", hasProtocol:true  },
  { id:"mock_p4",  name:"Shifani Ameena",       pid:"P004", adherence:65, lastSession:"2025-03-07", status:"Medium", sub:"AI Companion", condition:"Parkinson's",  hasProtocol:false },
  { id:"mock_p5",  name:"Percy Silva",          pid:"P005", adherence:88, lastSession:"2025-03-09", status:"High",   sub:"Standard",     condition:"Stroke",       hasProtocol:true  },
  { id:"mock_p6",  name:"Athula Premachandra",  pid:"P006", adherence:52, lastSession:"2025-03-08", status:"Low",    sub:"Standard",     condition:"TBI",          hasProtocol:true  },
  { id:"mock_p7",  name:"Aruni Perera",         pid:"P007", adherence:95, lastSession:"2025-03-09", status:"High",   sub:"AI Companion", condition:"Post-Surgery", hasProtocol:true  },
  { id:"mock_p8",  name:"Amal Mahendra",        pid:"P008", adherence:73, lastSession:"2025-03-08", status:"Medium", sub:"Standard",     condition:"Stroke",       hasProtocol:true  },
  { id:"mock_p9",  name:"Malkanthi Peris",      pid:"P009", adherence:25, lastSession:"2025-03-07", status:"Low",    sub:"Standard",     condition:"Parkinson's",  hasProtocol:false },
  { id:"mock_p10", name:"K.K. Muththukumaran",  pid:"P010", adherence:76, lastSession:"2025-03-08", status:"Medium", sub:"Standard",     condition:"TBI",          hasProtocol:true  },
  { id:"mock_p11", name:"Kamal Fernando",       pid:"P011", adherence:80, lastSession:"2025-03-09", status:"High",   sub:"AI Companion", condition:"Stroke",       hasProtocol:true  },
  { id:"mock_p12", name:"P.P. Sugathadasa",     pid:"P012", adherence:63, lastSession:"2025-03-08", status:"Medium", sub:"Standard",     condition:"Post-Surgery", hasProtocol:true  },
];
// ──────────────────────────────────────────────────────────────────────────────

interface DisplayPatient {
  id: string; name: string; pid: string; adherence: number;
  lastSession: string; status: 'High'|'Medium'|'Low';
  sub: string; condition: string; hasProtocol: boolean;
}

function adherenceColor(v: number) { return v >= 80 ? '#22c55e' : v >= 55 ? '#f97316' : '#ef4444'; }
function statusColor(s: string)    { return s === 'High' ? '#22c55e' : s === 'Medium' ? '#f97316' : '#ef4444'; }
...
export default function DoctorPatientsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (patients.length === 0) { setDisplayPatients([]); return; }
        ...
        snap.docs.forEach(d => { const d2 = d.data() as TherapyProtocol; protocolMap.set(d2.patientId,{...d2,id:d.id}); });

        setDisplayPatients(patients.map(p => {
          ...
          return {
            ...
            adherence,
            lastSession: lastDate ? lastDate.toLocaleDateString('en-CA') : 'No sessions',
            status: getStatus(adherence),
            sub: (p as any).subscriptionPlan==='ai_companion' ? 'AI Companion' : 'Standard',
  ...
  const filtered = useMemo(() => displayPatients.filter(p => {
    const q = search.toLowerCase();
    const matchQ = p.name.toLowerCase().includes(q) || p.pid.toLowerCase().includes(q);
    const matchA = adherenceFilter==='all' || p.status.toLowerCase()===adherenceFilter;
    const matchC = conditionFilter==='all' || p.condition.toLowerCase().replace(/[\s-]/g,'')===conditionFilter.toLowerCase().replace(/[\s-]/g,'');
    ...
    <div className="mpp" style={{ minHeight:'100vh', background:'#F0F4F8' }}>
      ...
      <div className="mpp-pad" style={{ maxWidth:1300, margin:'0 auto', padding:'32px 28px' }}>
        ...
        <div style={{ marginBottom:24, animation:'mppFadeUp .45s ease both', display:'flex', alignItems:'flex-start', justifyContent:'space-between' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            {USE_MOCK && (
              <div style={{ display:'flex', alignItems:'center', gap:6, background:'rgba(251,191,36,.08)', border:'1px solid rgba(251,191,36,.30)', borderRadius:10, padding:'7px 13px' }}>
        ...
        {error && <div style={{ marginBottom:16, padding:'12px 16px', background:'rgba(239,68,68,.06)', border:'1px solid rgba(239,68,68,.20)', borderRadius:12, fontSize:13, color:'#dc2626', fontWeight:500 }}>{error}</div>}
        ...
        {loading ? (
          ...
          <>
            <div className="mpp-stats" style={{ display:'flex', gap:12, marginBottom:22 }}>
              <StatCard label="Total Patients" value={displayPatients.length}                    color="#2DD4BF" delay={.05} icon={<User size={15}/>}/>
              <StatCard label="High Adherence" value={highCount}                                 color="#22c55e" delay={.10} icon={<Shield size={15}/>}/>
              <StatCard label="Need Attention" value={lowCount}                                  color="#ef4444" delay={.15} icon={<MessageCircle size={15}/>}/>
              <StatCard label="Protocols Set"  value={`${protoCount}/${displayPatients.length}`} color="#6366f1" delay={.20} icon={<ClipboardList size={15}/>}/>
            </div>

            <div className="mpp-filter-row" style={{ display:'flex', alignItems:'center', gap:14, marginBottom:18, animation:'mppFadeUp .45s ease .08s both' }}>
              <div style={{ position:'relative', width:270, flexShrink:0 }}>
                ...
                <input className="mpp-search" placeholder="Search by name or ID..." value={search} onChange={e => setSearch(e.target.value)}/>
              </div>
              <div style={{ flex:1 }}/>
            ...
            {displayPatients.length === 0 ? (
              ...
              <>
                <div style={{ background:'#fff', border:'2px solid #2DD4BF', borderRadius:18, overflow:'hidden', boxShadow:'0 4px 28px rgba(45,212,191,.10)', animation:'mppFadeUp .45s ease .14s both' }}>
                  <div className="mpp-scroll" style={{ overflowX:'auto' }}>
                    <table style={{ width:'100%', borderCollapse:'collapse', minWidth:980 }}>
                      <thead>
                        <tr><TH>Patient</TH><TH>ID</TH><TH>Condition</TH><TH>Adherence</TH><TH>Last Session</TH><TH>Status</TH><TH>Plan</TH><TH accent>Protocol</TH><TH>Message</TH><TH>Profile</TH></tr>
                      </thead>
                      <tbody>
                        {filtered.map((p, i) => {
                          ...
                          return (
                            <tr key={p.id} className="mpp-tr" style={{ animationDelay:`${.14+i*.035}s` }}>
                              ...
                              <td style={{ padding:'14px 16px', borderBottom:'1.5px dashed rgba(45,212,191,.22)', verticalAlign:'middle' }}>
                                <span className="mono" style={{ fontSize:12, color:'#2DD4BF', fontWeight:600, background:'rgba(45,212,191,.08)', border:'1px solid rgba(45,212,191,.18)', padding:'2px 8px', borderRadius:7 }}>{p.pid}</span>
                              </td>
                              <td style={{ padding:'14px 16px', borderBottom:'1.5px dashed rgba(45,212,191,.22)', verticalAlign:'middle' }}>
                              ...
                              <td style={{ padding:'14px 16px', borderBottom:'1.5px dashed rgba(45,212,191,.22)', verticalAlign:'middle' }}>
                                <Link href={`/doctor/patients/${p.id}/messages`} className="mpp-msg-btn"><MessageCircle size={12}/>Message</Link>
                              </td>
                              <td style={{ padding:'14px 16px', borderBottom:'1.5px dashed rgba(45,212,191,.22)', verticalAlign:'middle' }}>
                                <Link href={`/doctor/patients/${p.id}`} className="mpp-view-btn">View Profile</Link>
                              </td>
                            </tr>
                          );
                        })}
                ...
                <div style={{ marginTop:12, display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:8 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:6, padding:'5px 12px', borderRadius:99, background:'rgba(99,102,241,.08)', border:'1px solid rgba(99,102,241,.18)' }}>
