...
import {
  User as UserIcon, Mail, Phone, ClipboardList,
  Calendar, Loader2, Zap, Flame, Brain, Activity, Shield, 
  Heart, ArrowLeft,
} from "lucide-react";
...
const CSS = `
  ...
  /* Dark mode overrides (Optional if you want it to match Doctor Dash) */
  .dark .pf-card { background:#1e293b; border-color:rgba(51,65,85,0.9); box-shadow:0 2px 20px rgba(0,0,0,0.20); }
  .dark .pf-input { background:#0f172a; border-color:#334155; color:#f1f5f9; }
  .dark .stat-chip { background:#0f172a; border-color:#334155; }
`;

export default function DoctorViewPatientProfile() {
      ...
      try {
        ...
        if (snap.exists()) {
          ...
          setData({
            ...
            goals:             raw.goals || "No goals recorded.",
            bio:               raw.bio || "No bio provided.",
            condition:         raw.condition || "Not set",
            affectedSide:      raw.affectedSide || "Not specified",
  ...
  const XP_CAP = 5000;
  const xpPct = Math.min(100, (data.totalXp / XP_CAP) * 100);
      ...
      <main style={{maxWidth:1200,margin:"0 auto",padding:"0 24px",position:"relative",zIndex:1}}>
        ...
        <div style={{animation:"fadeUp 0.6s cubic-bezier(0.22,1,0.36,1) both",background:"#fff",borderRadius:22,padding:"20px 28px",marginBottom:24,border:"1.5px solid rgba(45,212,191,0.22)",boxShadow:"0 4px 32px rgba(11,30,51,0.07)",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:16,position:"relative",overflow:"hidden"}}>
          ...
          <div style={{position:"relative",zIndex:2}}>
            <p className="mono" style={{fontSize:9.5,color:"rgba(45,212,191,0.7)",textTransform:"uppercase",letterSpacing:"0.20em",marginBottom:4,fontWeight:600}}>Patient Preview</p>
            <h1 style={{fontSize:"clamp(1.5rem,2.6vw,2rem)",fontWeight:800,color:"#0B1E33",margin:0,lineHeight:1.15}}>
              {data.name}'s <span style={{color:"#2DD4BF"}}>Profile</span>
            </h1>
            <p style={{fontSize:13,color:"#94a3b8",marginTop:5,fontWeight:500}}>
              <span style={{color:"#2DD4BF",fontWeight:700}}>{data.unlockedLevels.length} levels</span> unlocked ·{" "}
              <span style={{color:"#8b5cf6",fontWeight:700}}>{data.totalXp} XP</span> earned
            </p>
          </div>
          ...
          <div style={{position:"relative",flexShrink:0}}>
            <div className="avatar-ring" style={{width:96,height:96,borderRadius:20,border:"2px solid #2DD4BF",background:"rgba(45,212,191,0.08)",overflow:"hidden",display:"flex",alignItems:"center",justifyContent:"center"}}>
              {data.profilePictureUrl
                ? <img src={data.profilePictureUrl} alt="avatar" style={{width:"100%",height:"100%",objectFit:"cover"}}/>
                : <UserIcon size={36} color="rgba(45,212,191,0.55)"/>}
            </div>
          </div>
          <div style={{flex:1,minWidth:220}}>
            <h2 style={{fontSize:"clamp(1.3rem,2.8vw,1.9rem)",fontWeight:800,color:"#0B1E33",margin:"0 0 3px",letterSpacing:"-0.01em"}}>{data.name}</h2>
            ...
            <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
              {[
                {icon:<Flame size={12}/>,  val:data.streak,                  label:"Streak",   c:"#f59e0b"},
                {icon:<Zap size={12}/>,    val:data.totalXp,                 label:"Total XP", c:"#2DD4BF"},
                {icon:<Brain size={12}/>,  val:data.unlockedLevels.length,   label:"Levels",   c:"#8b5cf6"},
                {icon:<Activity size={12}/>,val:data.completedSessions,      label:"Sessions", c:"#22c55e"},
              ].map(s => (
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 420px",gap:22,alignItems:"start"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:18}}>
            ...
            <div className="pf-card" style={{padding:28,animation:"cardPop 0.55s cubic-bezier(0.22,1,0.36,1) 0.10s both"}}>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:22}}>
                ...
                <div>
                  ...
                  <div className="mono" style={{fontSize:9,color:"#94a3b8",textTransform:"uppercase",letterSpacing:"0.14em",marginTop:1}}>Read Only View</div>
                </div>
              </div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
                <div style={{gridColumn:"1/-1"}}>
                  ...
                  <input className="pf-input" value={data.name} readOnly/>
                </div>
                <div style={{gridColumn:"1/-1"}}>
                  <div className="fl">Email Address</div>
                  <div style={{position:"relative"}}>
                    <input className="pf-input" value={data.email} readOnly style={{paddingLeft:36}}/>
                ...
                <div>
                  ...
                  <div style={{position:"relative"}}>
                    <input className="pf-input" value={data.phone} readOnly style={{paddingLeft:36}}/>
                    <Phone size={13} style={{position:"absolute",left:12,top:"50%",transform:"translateY(-50%)",color:"#94a3b8"}}/>
                  </div>
                </div>
                <div>
                  <div className="fl">Condition</div>
                  <input className="pf-input" value={data.condition} readOnly/>
                </div>
              </div>
              ...
              <div>
                <div className="fl">Affected Side</div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
                  {["Left","Right","Both"].map(s => (
                    <div key={s} 
                      style={{padding:"11px 0",borderRadius:11,fontSize:12,fontWeight:700,textAlign:"center",
                        border:`1.5px solid ${data.affectedSide===s?"#2DD4BF":"rgba(226,232,240,0.9)"}`,
                        background:data.affectedSide===s?"rgba(45,212,191,0.08)":"#f8fafc",
                        color:data.affectedSide===s?"#2DD4BF":"#64748b",
            ...
            <div className="pf-card" style={{padding:28,animation:"cardPop 0.55s cubic-bezier(0.22,1,0.36,1) 0.13s both"}}>
              <div className="fl" style={{marginBottom:10}}>Recovery Goals</div>
              <textarea className="pf-input" rows={3} value={data.goals} readOnly/>
            </div>

            {/* Bio */}
            <div className="pf-card" style={{padding:28,animation:"cardPop 0.55s cubic-bezier(0.22,1,0.36,1) 0.16s both"}}>
              <div className="fl" style={{marginBottom:10}}>About Patient</div>
              <textarea className="pf-input" rows={4} value={data.bio} readOnly/>
            </div>
          </div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:18}}>
            ...
            <div style={{animation:"cardPop 0.55s cubic-bezier(0.22,1,0.36,1) 0.11s both",background:"#0B1E33",borderRadius:22,border:"1px solid rgba(45,212,191,0.10)",boxShadow:"0 8px 36px rgba(11,30,51,0.18)",padding:"24px",position:"relative",overflow:"hidden"}}>
              ...
              <div style={{position:"relative",zIndex:2}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:20}}>
                  {[
                    {emoji:"🔥",label:"Streak",val:data.streak,unit:"DAYS",   c:"#f59e0b"},
                    {emoji:"⚡",label:"XP",     val:data.totalXp,unit:"POINTS",c:"#2DD4BF"},
                  ].map(s => (
                ...
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:7}}>
                  ...
                  <span className="mono" style={{fontSize:8,color:"#2DD4BF",fontWeight:700}}>{data.totalXp} / {XP_CAP}</span>
                </div>
                <div className="xp-bar-track">
                  <div className="xp-bar-fill" style={{width:`${xpPct}%`}}/>
                </div>
                <div style={{marginTop:12,display:"flex",justifyContent:"space-between"}}>
                  <span className="mono" style={{fontSize:8,color:"rgba(139,92,246,0.7)",letterSpacing:".10em"}}>{data.unlockedLevels.length}/5 LEVELS</span>
                  <span className="mono" style={{fontSize:8,color:"rgba(34,197,94,0.7)",letterSpacing:".10em"}}>{data.completedSessions} SESSIONS</span>
                </div>
              </div>
            </div>
            ...
            <div className="pf-card" style={{padding:24,animation:"cardPop 0.55s cubic-bezier(0.22,1,0.36,1) 0.13s both"}}>
              ...
              {[
                {icon:<ClipboardList size={15}/>, label:"Assigned Protocol", val:data.assignedProtocol, c:"#2DD4BF"},
                {icon:<Calendar size={15}/>,       label:"Next Appointment",  val:data.nextAppointment,   c:"#8b5cf6"},
                {icon:<Heart size={15}/>,         label:"Condition",         val:data.condition, c:"#ef4444"},
              ].map(item => (
            ...
            <div className="pf-card" style={{padding:24,animation:"cardPop 0.55s cubic-bezier(0.22,1,0.36,1) 0.16s both"}}>
              ...
              <div style={{display:"flex",gap:8}}>
                {[1,2,3,4,5].map(lvl => {
                  const done = data.unlockedLevels.includes(lvl);
                  return (
              ...
              <div className="mono" style={{marginTop:12,fontSize:9,color:"#94a3b8",textTransform:"uppercase",letterSpacing:".14em",textAlign:"center"}}>
                {data.unlockedLevels.length} of 5 levels unlocked
              </div>
            </div>
