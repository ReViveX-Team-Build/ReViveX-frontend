...
const MOCK_DB: MockP[] = [
  {
    ...
    joinDate: "2024-11-12",
    notes: [
      "Grip strength baseline 28 kg. Target 32 kg by end of month.",
      "Fatigue after >15 min sessions — reducing to 10 min with rest intervals.",
    ],
  },
  {
    uid: "mock_p2",
    ...
    joinDate: "2024-10-05",
    notes: [
      "Reaction time improved 480ms → 394ms. Cognitive accuracy 91%. Advancing to Level 3.",
    ],
  },
  {
    uid: "mock_p3",
    ...
    joinDate: "2025-01-18",
    notes: [
      "Post-op grip recovery progressing. No pain reported during Level 2.",
    ],
  },
  {
    uid: "mock_p4",
    ...
    monthlyTarget: 20,
    gameId: "precision_hold",
    level: 1,
    sessionsPerWeek: 4,
    difficulty: "easy",
    joinDate: "2024-12-03",
    notes: ["AI Companion daily check-ins showing strong engagement."],
  },
  {
    uid: "mock_p5",
    name: "Percy Silva",
    ...
    joinDate: "2024-09-22",
    notes: [
      "Motor reaction time down 22ms this month. Exceeding expectations.",
    ],
  },
  {
    uid: "mock_p6",
    ...
    joinDate: "2025-02-01",
    notes: [
      "Adherence declining — 2 consecutive missed sessions.",
      "Prefers morning sessions. Schedule updated.",
    ],
  },
  {
    uid: "mock_p7",
    ...
    joinDate: "2024-10-30",
    notes: [
      "Best performer. Level 4 evaluation Friday.",
      "AI Companion interaction rate 87%.",
    ],
  },
  {
    uid: "mock_p8",
    ...
    difficulty: "medium",
    joinDate: "2025-01-07",
    notes: ["Steady performance. Motivational check-in recommended."],
  },
  {
    uid: "mock_p9",
  ...
  },
  {
    ...
    joinDate: "2024-12-15",
    notes: [
      "Cognitive accuracy trending up. Mild endurance drop in final 10 min.",
    ],
  },
  {
    uid: "mock_p11",
    ...
    difficulty: "hard",
    joinDate: "2024-10-18",
    notes: ["80%+ adherence for 6 consecutive weeks."],
  },
  {
    uid: "mock_p12",
    ...
    difficulty: "medium",
    joinDate: "2025-01-25",
    notes: ["Post-surgery pain resolved. Grip endurance improving weekly."],
  },
];
// ──────────────────────────────────────────────────────────────────────────────
...
function statusColor(s: string) {
  return s === "High" ? "#10b981" : s === "Medium" ? "#f59e0b" : "#ef4444";
}
function getStatus(a: number) {
...
const CSS = `
  ...
  /* ── Dark mode overrides ── */
  .dark .pp-card { background: #1e293b; border-color: #334155; box-shadow: 0 2px 18px rgba(0,0,0,.20); }
  .dark .pp-card:hover { box-shadow: 0 18px 52px rgba(0,0,0,.35) !important; }
  .dark .pp-back-btn { background: #1e293b; color: #94a3b8; border-color: #334155; }
  .dark .pp-back-btn:hover { background: #334155; color: #f1f5f9; border-color: #475569; }
  .dark .pp-note { color: #94a3b8; }
`;
...
function AnimBar({
  ...
  return (
    <div
      style={{
        height: 8,
        background: isDark ? "rgba(241,245,249,.10)" : "rgba(11,30,51,.07)",
        borderRadius: 99,
        overflow: "hidden",
      }}>
      <div
...
export default function PatientProfilePage() {
  ...
  useEffect(() => {
    ...
    if (USE_MOCK) {
      ...
      setAdherence(m.adherence);
      setStatus(m.status);
      setStreak(m.streak);
      setCompleted(m.completedSessions);
      setTarget(m.monthlyTarget);
      setLastSess(m.lastSession);
      setIsAI(m.isAIPlan);
      setProtocol({
        gameId: m.gameId,
        level: m.level,
    ...
    (async () => {
      try {
        ...
        const completedSessions = sessionData.filter(
          (s) => s.durationSeconds > 60,
        ).length;
        const adh = Math.min(
          100,
          Math.round((completedSessions / monthlyTarget) * 100),
        );

        setName(patientData.name);
        setPid(
          (patientData as any).patientId ??
            patientData.uid.slice(0, 7).toUpperCase(),
        );
        ...
        if (protocolData)
          setProtocol({
            gameId: protocolData.gameId ?? "",
            level: protocolData.level ?? 1,
            sessionsPerWeek: protocolData.sessionsPerWeek ?? 5,
            difficulty: protocolData.settings?.difficulty ?? "",
          });
      ...
      <div
        ...
        <div
          style={{
            ...
            height: 700,
            background:
              "radial-gradient(circle,rgba(45,212,191,.055),transparent 65%)",
            borderRadius: "50%",
          }}
        />
        <div
          style={{
            position: "absolute",
            inset: 0,
            backgroundImage: `linear-gradient(${isDark ? "rgba(241,245,249,.018)" : "rgba(11,30,51,.022)"} 1px,transparent 1px),linear-gradient(90deg,${isDark ? "rgba(241,245,249,.018)" : "rgba(11,30,51,.022)"} 1px,transparent 1px)`,
      ...
      <main
        ...
        <div
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            ...
            <Link
              href={`/doctor/patients/${patientId}/messages`}
        ...
        <div
          style={{
            animation: "ppCardPop .55s cubic-bezier(.22,1,.36,1) .05s both",
            background: isDark ? "#1e293b" : "#fff",
            borderRadius: 22,
            border: "1.5px solid rgba(45,212,191,.22)",
            boxShadow: isDark
              ? "0 4px 28px rgba(0,0,0,.25)"
              : "0 4px 28px rgba(11,30,51,.07)",
            padding: "26px 28px",
            marginBottom: 20,
            position: "relative",
            overflow: "hidden",
          ...
          <div
            style={{
              ...
              height: 180,
              background:
                "radial-gradient(circle,rgba(45,212,191,.07),transparent 70%)",
            }}
          />
          <div
            style={{
            ...
            <div
              style={{
                ...
                inset: 0,
                background:
                  "linear-gradient(90deg,transparent,rgba(45,212,191,.04),transparent)",
                animation: "ppShimmer 5s ease-in-out infinite",
              }}
            />
          </div>
          <div
            ...
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
              ...
              <div
                ...
                {isAI ? (
                  ...
                  <Shield size={13} color={isDark ? "#94a3b8" : "#64748b"} />
                )}
                <span
                  style={{
                    fontSize: 12,
                    fontWeight: 700,
                    color: isAI ? "#2DD4BF" : isDark ? "#94a3b8" : "#64748b",
                  }}>
                  {isAI ? "AI Companion" : "Standard"}
                </span>
              </div>
            </div>
          </div>
        ...
        {/* Stats Grid */}
        <div
          className="pp-stat-grid"
          style={{
            display: "grid",
            gridTemplateColumns: "repeat(4,1fr)",
            gap: 14,
            marginBottom: 20,
            animation: "ppCardPop .55s cubic-bezier(.22,1,.36,1) .12s both",
          }}>
          {[
            {
              label: "Adherence",
                ...
                <div
                  ...
                  {"dateVal" in s && s.dateVal ? (
                    ...
                    <span style={{ fontSize: 20 }}>
                      <AnimNum to={s.val} suffix={s.suffix} delay={300} />
                    </span>
                  )}
                </div>
              </div>
        ...
        <div
          ...
          <div
            ...
            style={{
              padding: "22px",
              animation: "ppCardPop .55s cubic-bezier(.22,1,.36,1) .20s both",
            }}>
            <div
              style={{
                display: "flex",
                ...
                <div>
                  ...
                  <div
                    style={{
                      ...
                      color: aColor,
                      lineHeight: 1,
                      textShadow: `0 0 20px ${aColor}40`,
                    }}>
                    <AnimNum to={adherence} suffix="%" delay={400} />
                  </div>
                </div>
                ...
                {Array.from({ length: target }).map((_, i) => (
                  <div
                    ...
                    style={{
                      ...
                      background:
                        i < completed
                          ? `linear-gradient(90deg,${aColor}cc,${aColor})`
                          : "rgba(45,212,191,.10)",
                      boxShadow: i < completed ? `0 0 5px ${aColor}60` : "none",
                      transition: "all .3s",
                    }}
                  />
                ))}
            ...
            {protocol && (
              <div
                ...
                <div
                  ...
                  · Level {protocol.level} · {protocol.sessionsPerWeek}x/week ·{" "}
                  <span
          ...
          <div
            ...
            style={{
              padding: "22px",
              animation: "ppCardPop .55s cubic-bezier(.22,1,.36,1) .28s both",
            }}>
            <div
              style={{
                display: "flex",
              ...
              <div>
                ...
                <p
                  ...
                  {USE_MOCK ? "Sample notes" : "Saved as feedback messages"}
                </p>
              </div>
            </div>
            <div
              ...
              {notes.length === 0 ? (
                ...
                notes.map((note, i) => (
                  <div
                    ...
                    className="pp-note"
                    style={{
                      animation: "ppCardPop .4s ease both",
            ...
            <div
              style={{
                ...
                background: isDark
                  ...
                  : "rgba(240,244,248,.7)",
                borderRadius: 14,
                border: "1px dashed rgba(45,212,191,.20)",
              }}>
              <textarea
                placeholder={
                  USE_MOCK
                    ? "Add a note… (demo — not saved to database)"
                    : "Add a clinical note… (saved as feedback to patient)"
                }
                rows={3}
                value={noteInput}
              ...
              <div
                ...
                <button
                  ...
                  style={{
                    padding: "8px 16px",
                    borderRadius: 10,
                    background: noteInput.trim()
                      ? "rgba(45,212,191,.10)"
                      : isDark
                        ? "rgba(71,85,105,.4)"
                        : "rgba(226,232,240,.6)",
                    border: noteInput.trim()
