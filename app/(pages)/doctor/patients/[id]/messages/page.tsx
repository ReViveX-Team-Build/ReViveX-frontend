...
import { ArrowLeft, Send, Search, Bot, User, MessageCircle, AlertCircle, Activity, Sparkles, Lock, CheckCircle2, ChevronRight, Zap, Shield, ToggleLeft, ToggleRight, Info, Crown, Loader2 } from 'lucide-react';
...
interface SidebarPatient { uid:string; name:string; pid:string; condition:string; status:'High'|'Medium'|'Low'; adherence:number; isAIPlan:boolean; }

const MOCK_SIDEBAR: SidebarPatient[] = [
  { uid:"mock_p1",  name:"P.B. De Silva",        pid:"P001", condition:"Stroke",       status:"Low",    adherence:45, isAIPlan:false },
  { uid:"mock_p2",  name:"Anura Dissanayaka",    pid:"P002", condition:"TBI",          status:"High",   adherence:92, isAIPlan:true  },
  { uid:"mock_p3",  name:"Sarath Watawala",      pid:"P003", condition:"Post-Surgery", status:"High",   adherence:78, isAIPlan:false },
  { uid:"mock_p4",  name:"Shifani Ameena",       pid:"P004", condition:"Parkinson's",  status:"Medium", adherence:65, isAIPlan:true  },
  { uid:"mock_p5",  name:"Percy Silva",          pid:"P005", condition:"Stroke",       status:"High",   adherence:88, isAIPlan:false },
  { uid:"mock_p6",  name:"Athula Premachandra",  pid:"P006", condition:"TBI",          status:"Low",    adherence:52, isAIPlan:false },
  { uid:"mock_p7",  name:"Aruni Perera",         pid:"P007", condition:"Post-Surgery", status:"High",   adherence:95, isAIPlan:true  },
  { uid:"mock_p8",  name:"Amal Mahendra",        pid:"P008", condition:"Stroke",       status:"Medium", adherence:73, isAIPlan:false },
  { uid:"mock_p9",  name:"Malkanthi Peris",      pid:"P009", condition:"Parkinson's",  status:"Low",    adherence:25, isAIPlan:false },
  { uid:"mock_p10", name:"K.K. Muththukumaran",  pid:"P010", condition:"TBI",          status:"Medium", adherence:76, isAIPlan:false },
  { uid:"mock_p11", name:"Kamal Fernando",       pid:"P011", condition:"Stroke",       status:"High",   adherence:80, isAIPlan:true  },
  { uid:"mock_p12", name:"P.P. Sugathadasa",     pid:"P012", condition:"Post-Surgery", status:"Medium", adherence:63, isAIPlan:false },
];

const MOCK_CHATS: Record<string,{sender:'doctor'|'patient';text:string;time:string}[]> = {
  mock_p1:[{sender:'patient',text:"Doctor, I've been struggling with the grip exercises. My hand tires out quickly.",time:"09:14"},{sender:'doctor',text:"That's completely normal at this stage. Try resting 90 seconds between sets — we can also reduce target force by 10%.",time:"09:22"},{sender:'patient',text:"Should I still aim for all 5 sessions this week?",time:"09:25"},{sender:'doctor',text:"Yes — keep the frequency, reduce intensity. Consistency matters more than force right now.",time:"09:31"},{sender:'patient',text:"Thank you doctor. I'll try today.",time:"09:33"}],
  mock_p2:[{sender:'patient',text:"I finished all 5 sessions this week! The Rhythm Reef game is really enjoyable.",time:"10:05"},{sender:'doctor',text:"Excellent work, Anura! Your reaction time improved 18% this month — exceptional for TBI recovery.",time:"10:18"},{sender:'patient',text:"The AI companion tips have been very helpful.",time:"10:20"},{sender:'doctor',text:"Wonderful. Advancing you to Level 3 starting Monday. Keep this momentum.",time:"10:24"}],
  mock_p4:[{sender:'patient',text:"The tremors are worse in the mornings. Should I do sessions in the afternoon?",time:"14:02"},{sender:'doctor',text:"Yes, try afternoon sessions when tremors are more settled. Very common with Parkinson's — I'll note this.",time:"14:15"},{sender:'patient',text:"Thank you, that makes sense.",time:"14:18"}],
  mock_p6:[{sender:'doctor',text:"Athula, I noticed you've missed a few sessions. Is everything okay?",time:"11:00"},{sender:'patient',text:"Very tired after work. Hard to find motivation in the evenings.",time:"11:14"},{sender:'doctor',text:"Let's try shorter 8-minute sessions for this week to keep the streak going. Small progress is still progress.",time:"11:19"},{sender:'patient',text:"I'll try that. Thank you.",time:"11:22"}],
  mock_p7:[{sender:'patient',text:"Completed session 19 this morning! Feeling really strong.",time:"08:02"},{sender:'doctor',text:"Aruni, 95% adherence this month is remarkable. Level 4 evaluation is scheduled for Friday.",time:"08:15"},{sender:'patient',text:"I'm excited! The AI companion suggested I work on my pinch grip too.",time:"08:18"},{sender:'doctor',text:"Yes, great addition. I'll update your protocol at the next review.",time:"08:22"}],
  mock_p9:[{sender:'doctor',text:"Malkanthi, only 5 sessions completed this month. Is everything okay?",time:"11:00"},{sender:'patient',text:"I've been having bad tremor days. Some mornings I can't hold the device.",time:"11:14"},{sender:'doctor',text:"Adjusting your protocol to shorter 8-minute sessions. Try one today when you feel ready.",time:"11:19"},{sender:'patient',text:"I will try. Thank you for understanding.",time:"11:22"}],
};
function getMockChat(uid:string) {
  return MOCK_CHATS[uid] ?? [{sender:'doctor' as const,text:`Hello ${MOCK_SIDEBAR.find(p=>p.uid===uid)?.name.split(' ')[0]??'there'}! How are your therapy sessions going this week?`,time:'09:00'},{sender:'patient' as const,text:"Things are going well, thank you doctor.",time:"09:05"}];
}
// ──────────────────────────────────────────────────────────────────────────────

type ActivePanel = 'chat'|'instruction'|'feedback'|'ai_generate';
interface ChatMessage { sender:'doctor'|'patient'; text:string; time:string; }
interface SentItem { id:string; type:'instruction'|'feedback'|'ai_insight'; title:string; content:string; time:string; isImportant:boolean; sentByAI:boolean; }
...
function initials(n:string){ return n.split(' ').map(w=>w[0]).slice(0,2).join(''); }
function commToChat(c:Communication,docId:string):ChatMessage{ return {sender:c.senderId===docId?'doctor':'patient',text:c.content,time:(c.timestamp as Timestamp).toDate().toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}; }
...
export default function DoctorMessagingHub() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!patients.length) { setSidebarPatients([]); return; }
        const uids = patients.map(p => p.uid);
        const weekSessions = await getCohortSessionsThisWeek(uids);
        const snap = await getDocs(query(collection(db,'protocols'),where('doctorId','==',user.uid)));
        const protocolMap = new Map<string,number>();
        snap.docs.forEach(d=>{ const d2=d.data(); protocolMap.set(d2.patientId, d2.sessionsPerWeek??5); });
        setSidebarPatients(patients.map(p=>{
          const spw = protocolMap.get(p.uid)??5;
          const completed = weekSessions.filter(s=>s.userId===p.uid&&s.durationSeconds>60).length;
          const adherence = Math.min(100,Math.round((completed/spw)*100));
          return { uid:p.uid, name:p.name, pid:(p as any).patientId??p.uid.slice(0,7).toUpperCase(), condition:p.condition, status:getStatus(adherence), adherence, isAIPlan:(p as any).subscriptionPlan==='ai_companion' };
        }));
  ...
  useEffect(() => {
    ...
    setChatLoading(true); setChatMessages([]);

    if (USE_MOCK) {
      // Mock: fake 380ms delay then load seeded messages
      const t = setTimeout(() => { setChatMessages(getMockChat(selectedId)); setChatLoading(false); }, 380);
      return () => clearTimeout(t);
    }

    // Real path
    if (!user) return;
    const unsub = subscribeToDirect(selectedId, user.uid, (comms) => {
      setChatMessages(comms.map(c=>commToChat(c, user.uid)));
  ...
  const filtered = sidebarPatients.filter(p => p.name.toLowerCase().includes(search.toLowerCase()) || p.pid.toLowerCase().includes(search.toLowerCase()));
  ...
  const sendChat = async () => {
    ...
    try { await sendDirectMessage(user!.uid, selectedId, text); }
    catch(e){ console.error(e); }
    finally { setSending(false); setTimeout(()=>chatInputRef.current?.focus(),50); }
  };

  const handleChatKey = (e:React.KeyboardEvent) => { if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();sendChat();} };
  ...
  const sendInstruction = async () => {
    ...
    setInstrTitle(''); setInstrContent(''); setInstrImportant(false);
    if (!USE_MOCK && user) { try { await sendFromDoctor(user.uid,selectedId,'instruction',item.title,item.content,item.isImportant); } catch(e){console.error(e);} }
  };
  ...
  const generateAI = useCallback(async () => {
    ...
    try {
      ...
      setAiDraft(text);
      setAiTitle(aiMode==='feedback' ? `AI Feedback — ${patient.name.split(' ')[0]} (${today})` : `AI Protocol Note — ${patient.name.split(' ')[0]}`);
    } catch(e){ console.error(e); setAiDraft('Failed to generate. Please try again.'); }
  ...
  const sendAiDraft = async () => {
    ...
    const time = new Date().toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'});
    const item:SentItem = {id:Date.now().toString(),type:aiMode==='feedback'?'ai_insight':'instruction',title:aiTitle,content:aiDraft,time,isImportant:false,sentByAI:true};
    setAiSent(prev=>({...prev,[selectedId]:[item,...(prev[selectedId]??[])]}));
    setAiDraft(''); setAiTitle('');
    if (!USE_MOCK && user) { try { const t=aiMode==='feedback'?'ai_insight' as const:'instruction' as const; await sendFromDoctor(user.uid,selectedId,t,item.title,item.content); } catch(e){console.error(e);} }
  };
    ...
    <div style={{display:'flex',flexDirection:'column',flex:1,minHeight:0}}>
        ...
        {chatLoading ? (
          ...
          <>
            ...
            {chatMessages.map((msg,i)=>{
              ...
              return (
                <div key={i} style={{display:'flex',justifyContent:isDoc?'flex-end':'flex-start',gap:8,alignItems:'flex-end'}}>
      ...
      <div style={{padding:'12px 18px',borderTop:'1px solid rgba(226,232,240,.8)',background:'#fafbfd',display:'flex',alignItems:'center',gap:9}}>
        ...
        <button className="dm-send-btn" onClick={sendChat} disabled={!chatInput.trim()||!patient||sending}>
          {sending?<Loader2 size={16} style={{animation:'dmSpin 1s linear infinite'}}/>:<Send size={16}/>}
        </button>
      </div>
    </div>
  );
  ...
  const ComposeForm = ({typeLabel,typeColor,title,setTitle,content,setContent,important,setImportant,onSend,sentItems,hint}:{typeLabel:string;typeColor:string;title:string;setTitle:(v:string)=>void;content:string;setContent:(v:string)=>void;important:boolean;setImportant:(v:boolean)=>void;onSend:()=>void;sentItems:SentItem[];hint:string}) => (
    <div className="dm-scroll" style={{flex:1,overflowY:'auto',padding:'20px 22px',display:'flex',flexDirection:'column',gap:14}}>
      ...
      <div>
        ...
        <input className="dm-title-input" value={title} onChange={e=>setTitle(e.target.value)} placeholder={typeLabel==='Instruction'?'e.g. Protocol Adjustment — Week 4':'e.g. Weekly Progress Summary'}/>
      </div>
      <div>
        <label className="mono" style={{display:'block',fontSize:9,color:'#94a3b8',textTransform:'uppercase',letterSpacing:'0.14em',fontWeight:700,marginBottom:7}}>Content</label>
        <textarea className="dm-textarea" rows={6} value={content} onChange={e=>setContent(e.target.value)} placeholder={`Write your ${typeLabel.toLowerCase()} for ${patient?.name.split(' ')[0]??'patient'}…`}/>
      </div>
      ...
      {sentItems.length>0&&(
        <div style={{marginTop:4}}>
          <div className="mono" style={{fontSize:9,color:'#94a3b8',textTransform:'uppercase',letterSpacing:'0.14em',marginBottom:10,fontWeight:700}}>Sent This Session</div>
          {sentItems.map(item=>(
            <div key={item.id} className="dm-sent-item" style={{marginBottom:8}}>
  ...
  const AiPanel = () => (
    <div className="dm-scroll" style={{flex:1,overflowY:'auto',padding:'20px 22px',display:'flex',flexDirection:'column',gap:16,position:'relative'}}>
      {!canUseAI&&(
      ...
      <button className="dm-ai-btn" onClick={generateAI} disabled={aiLoading||!canUseAI} style={{width:'100%',opacity:!canUseAI?.45:1}}>
        ...
        {aiLoading?'Generating with Gemini…':`Generate AI ${aiMode.charAt(0).toUpperCase()+aiMode.slice(1)}`}
      </button>
      {aiDraft&&(
        <div style={{animation:'dmCardPop .45s cubic-bezier(.22,1,.36,1) both'}}>
          <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}>
            <div style={{width:6,height:6,borderRadius:'50%',background:'#6366f1',boxShadow:'0 0 6px rgba(99,102,241,.7)',animation:'dmDot 2s ease-in-out infinite'}}/>
          ...
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginTop:10,flexWrap:'wrap',gap:8}}>
            ...
            <button className="dm-compose-btn" onClick={sendAiDraft} style={{background:'linear-gradient(135deg,#6366f1,#8b5cf6)',color:'#fff',boxShadow:'0 6px 22px rgba(99,102,241,.32)',padding:'11px 20px'}}>
              <Send size={13} style={{position:'relative',zIndex:1}}/><span style={{position:'relative',zIndex:1}}>Send to Patient{USE_MOCK?' (Demo)':''}</span>
            </button>
          </div>
        </div>
      )}
  ...
  return (
    <div className="dm" style={{height:'100vh',background:'#F0F4F8',display:'flex',flexDirection:'column',padding:'20px',gap:14,overflow:'hidden'}}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:12,animation:'dmFadeUp .45s ease both',flexShrink:0}}>
        ...
        {USE_MOCK&&(
          <div style={{display:'flex',alignItems:'center',gap:6,background:'rgba(251,191,36,.08)',border:'1px solid rgba(251,191,36,.25)',borderRadius:10,padding:'5px 12px'}}>
            <div style={{width:5,height:5,borderRadius:'50%',background:'#fbbf24',boxShadow:'0 0 5px #fbbf24'}}/>
            <span className="mono" style={{fontSize:9,color:'#fbbf24',fontWeight:700,textTransform:'uppercase',letterSpacing:'0.10em'}}>Demo Data</span>
          </div>
        )}
      </div>
      ...
      <div style={{display:'flex',gap:14,flex:1,minHeight:0,animation:'dmFadeUp .50s ease .06s both'}}>
          ...
          <div className="dm-sidebar" style={{flex:1,overflowY:'auto',padding:'8px'}}>
            {patientsLoading ? (
              <div style={{display:'flex',alignItems:'center',justifyContent:'center',padding:'32px 0',gap:8}}>
                <Loader2 size={18} color="#2DD4BF" style={{animation:'dmSpin 1s linear infinite'}}/><span style={{fontSize:12,color:'#94a3b8'}}>Loading…</span>
              </div>
            ) : filtered.map(p=>{
              const sc=statusColor(p.status), ac=adherenceColor(p.adherence), isA=p.uid===selectedId;
              return (
                <button key={p.uid} className={`dm-patient-btn${isA?' active':''}`} onClick={()=>setSelectedId(p.uid)}>
        ...
        <div className="dm-main-col" style={{flex:1,display:'flex',flexDirection:'column',minWidth:0,background:'#fff',borderRadius:20,border:'1px solid rgba(226,232,240,.9)',boxShadow:'0 2px 18px rgba(11,30,51,.055)',overflow:'hidden'}}>
          {patient ? (
            <>
              {/* Hero header */}
              <div style={{background:'#0B1E33',position:'relative',overflow:'hidden',flexShrink:0}}>
                <div style={{position:'absolute',inset:0,pointerEvents:'none',backgroundImage:'linear-gradient(rgba(45,212,191,.04) 1px,transparent 1px),linear-gradient(90deg,rgba(45,212,191,.04) 1px,transparent 1px)',backgroundSize:'32px 32px'}}/>
                <div style={{position:'absolute',inset:0,overflow:'hidden',pointerEvents:'none'}}><div style={{position:'absolute',left:0,right:0,height:'22%',background:'linear-gradient(to bottom,transparent,rgba(45,212,191,.05),transparent)',animation:'dmScanLine 5.5s linear infinite'}}/></div>
                <div style={{position:'relative',zIndex:2,padding:'18px 22px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:16,flexWrap:'wrap'}}>
                  <div style={{display:'flex',alignItems:'center',gap:14}}>
                    <div style={{position:'relative'}}>
                      <div style={{width:48,height:48,borderRadius:14,background:`linear-gradient(135deg,${aColor},${aColor}aa)`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:14,fontWeight:800,color:'#fff',boxShadow:`0 0 0 2.5px ${aColor}40,0 5px 18px ${aColor}35`,animation:'dmGlow 3s ease-in-out infinite',flexShrink:0}}>
                    ...
                    <div>
                      <p className="mono" style={{fontSize:8.5,color:'rgba(45,212,191,.60)',textTransform:'uppercase',letterSpacing:'0.20em',marginBottom:2}}>Active Patient</p>
                      <div style={{fontSize:17,fontWeight:800,color:'#fff',lineHeight:1.2}}>{patient.name}</div>
                      <div style={{display:'flex',alignItems:'center',gap:8,marginTop:4,flexWrap:'wrap'}}>
                        <span className="mono" style={{fontSize:10,color:'#2DD4BF',background:'rgba(45,212,191,.12)',border:'1px solid rgba(45,212,191,.20)',padding:'1px 8px',borderRadius:7}}>{patient.pid}</span>
                  ...
                  <div style={{display:'flex',gap:9,flexWrap:'wrap',alignItems:'center'}}>
                    {patient.isAIPlan
                      ? <div style={{display:'flex',alignItems:'center',gap:7,background:'rgba(45,212,191,.10)',border:'1px solid rgba(45,212,191,.22)',borderRadius:11,padding:'7px 13px'}}><Bot size={13} color="#2DD4BF"/><span style={{fontSize:11.5,fontWeight:700,color:'#2DD4BF'}}>AI Companion</span></div>
                      : <div style={{display:'flex',alignItems:'center',gap:7,background:'rgba(255,255,255,.06)',border:'1px solid rgba(255,255,255,.09)',borderRadius:11,padding:'7px 13px'}}><Shield size={13} color="rgba(255,255,255,.35)"/><span style={{fontSize:11.5,fontWeight:700,color:'rgba(255,255,255,.40)'}}>Standard</span></div>
                    }
                    <Link href={`/doctor/patients/${selectedId}`} style={{display:'inline-flex',alignItems:'center',gap:5,padding:'7px 13px',borderRadius:11,background:'rgba(255,255,255,.06)',border:'1px solid rgba(255,255,255,.09)',fontSize:11.5,fontWeight:700,color:'rgba(255,255,255,.55)',textDecoration:'none'}}>
              ...
              <div style={{display:'flex',gap:6,padding:'10px 16px',borderBottom:'1px solid rgba(226,232,240,.8)',background:'rgba(248,250,252,.95)',flexShrink:0}}>
                {[
                  ...
                  {id:'feedback'    as ActivePanel,icon:<Activity size={16}/>,    label:'Feedback',   color:{active:'rgba(45,212,191,.09)',border:'rgba(45,212,191,.28)',text:'#0891b2'}},
                  {id:'ai_generate' as ActivePanel,icon:<Sparkles size={16}/>,    label:'AI Generate',color:{active:'linear-gradient(135deg,rgba(99,102,241,.10),rgba(139,92,246,.07))',border:'rgba(99,102,241,.28)',text:'#6366f1'},ai:true},
                ].map(tab=>(
                  <button key={tab.id} className={`dm-tab${panel===tab.id?' active':''}`} onClick={()=>setPanel(tab.id)} style={{background:panel===tab.id?tab.color.active:'transparent',border:`1.5px solid ${panel===tab.id?tab.color.border:'transparent'}`,color:panel===tab.id?tab.color.text:'#94a3b8',position:'relative'}}>
                    {tab.icon}
                    <span className="dm-tab-label mono" style={{fontSize:8.5,fontWeight:800,textTransform:'uppercase',letterSpacing:'0.12em'}}>{tab.label}</span>
                    {tab.ai&&<>
                      <div style={{position:'absolute',top:4,right:4,width:14,height:14,borderRadius:'50%',background:'linear-gradient(135deg,#6366f1,#8b5cf6)',display:'flex',alignItems:'center',justifyContent:'center',boxShadow:'0 2px 6px rgba(99,102,241,.45)'}}><Crown size={7} color="#fff"/></div>
                      {!canUseAI&&<div style={{position:'absolute',top:4,left:4,width:12,height:12,borderRadius:'50%',background:'#ef4444',display:'flex',alignItems:'center',justifyContent:'center'}}><Lock size={7} color="#fff"/></div>}
                    </>}
                  </button>
                ))}
              </div>
              ...
              {panel==='instruction' && <ComposeForm typeLabel="Instruction" typeColor="#f59e0b" title={instrTitle} setTitle={setInstrTitle} content={instrContent} setContent={setInstrContent} important={instrImportant} setImportant={setInstrImportant} onSend={sendInstruction} sentItems={instrSent[selectedId]??[]} hint={`Write a clinical instruction for ${patient.name.split(' ')[0]}. Instructions are highlighted and require acknowledgment.`}/>}
              {panel==='feedback' && <ComposeForm typeLabel="Feedback" typeColor="#2DD4BF" title={fbTitle} setTitle={setFbTitle} content={fbContent} setContent={setFbContent} important={fbImportant} setImportant={setFbImportant} onSend={sendFeedback} sentItems={fbSent[selectedId]??[]} hint={`Send clinical feedback to ${patient.name.split(' ')[0]} based on their recent session performance.`}/>}
              {panel==='ai_generate' && <AiPanel/>}
            </>
          ) : (
            <div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',flexDirection:'column',gap:14}}>
              {patientsLoading
                ? <><Loader2 size={28} color="#2DD4BF" style={{animation:'dmSpin 1s linear infinite'}}/><span style={{fontSize:13,color:'#94a3b8'}}>Loading…</span></>
                : <><MessageCircle size={36} color="#cbd5e1"/><p style={{fontSize:14,fontWeight:600,color:'#94a3b8'}}>Select a patient to start messaging</p></>
              }
            </div>
          )}
        </div>
