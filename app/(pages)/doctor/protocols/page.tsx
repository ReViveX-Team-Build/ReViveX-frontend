...
import {
  ...
  Play, Send, Activity, BookOpen, Users,
  ChevronDown, Check, ArrowLeft, Search,
  Bot, Shield, ClipboardList, User,
...
═══════════════════════════════════════════════════════════ */
const ALL_PATIENTS = [
  { id: '1',  name: 'P.B. De Silva',        pid: 'P001', condition: 'Stroke',       status: 'Low',    sub: 'Standard',     adherence: 45,  hasProtocol: false },
  { id: '2',  name: 'Anura Dissanayaka',    pid: 'P002', condition: 'TBI',           status: 'High',   sub: 'AI Companion', adherence: 92,  hasProtocol: true  },
  { id: '3',  name: 'Saeath Watawala',      pid: 'P003', condition: 'Stroke',        status: 'Medium', sub: 'AI Companion', adherence: 78,  hasProtocol: true  },
  { id: '4',  name: 'Shifani Ameena',       pid: 'P004', condition: 'Post-Surgery',  status: 'Medium', sub: 'Standard',     adherence: 65,  hasProtocol: false },
  { id: '5',  name: 'Percy Silva',          pid: 'P005', condition: 'TBI',           status: 'High',   sub: 'AI Companion', adherence: 88,  hasProtocol: true  },
  { id: '6',  name: 'Athula Premachandra',  pid: 'P006', condition: 'Stroke',        status: 'Low',    sub: 'Standard',     adherence: 52,  hasProtocol: false },
  { id: '7',  name: 'Aruni Perera',         pid: 'P007', condition: 'Post-Surgery',  status: 'High',   sub: 'AI Companion', adherence: 95,  hasProtocol: true  },
  { id: '8',  name: 'Amal Mahendra',        pid: 'P008', condition: 'TBI',           status: 'Medium', sub: 'Standard',     adherence: 73,  hasProtocol: true  },
  { id: '9',  name: 'Malkanthi Peris',      pid: 'P009', condition: 'Stroke',        status: 'Low',    sub: 'Standard',     adherence: 25,  hasProtocol: false },
  { id: '10', name: 'K.K. Muththukumaran',  pid: 'P010', condition: 'TBI',           status: 'High',   sub: 'AI Companion', adherence: 76,  hasProtocol: true  },
  { id: '11', name: 'Kamal Fernando',       pid: 'P011', condition: 'Post-Surgery',  status: 'High',   sub: 'AI Companion', adherence: 80,  hasProtocol: true  },
  { id: '12', name: 'P.P. Sugathadasa',     pid: 'P012', condition: 'Stroke',        status: 'High',   sub: 'AI Companion', adherence: 63,  hasProtocol: false },
];
...
const GameCanvas: React.FC<{ pressure: number; hand: string }> = ({ pressure, hand }) => {
  ...
  useEffect(() => {
    ...
    /* ── visibility pause ── */
    const onVisibility = () => { g.paused = document.hidden; };
    ...
    const draw = () => {
      ...
      ctx.fillStyle = glowR;
      ctx.beginPath(); ctx.arc(ballX, g.ballY, 28, 0, Math.PI * 2); ctx.fill();
      ...
      ctx.fillStyle = ballG;
      ctx.beginPath(); ctx.arc(ballX, g.ballY, 18, 0, Math.PI * 2); ctx.fill();

      // Shine
      ctx.fillStyle = 'rgba(255,255,255,0.32)';
      ctx.beginPath(); ctx.ellipse(ballX - 5, g.ballY - 7, 7, 5, -0.5, 0, Math.PI * 2); ctx.fill();
...
function MiniBar({ value, color }: { value: number; color: string }) {
  ...
  return (
    <div style={{ width: 42, height: 4, background: 'rgba(11,30,51,0.08)', borderRadius: 99, overflow: 'hidden', flexShrink: 0 }}>
...
function TherapyProtocolsInner() {
  ...
  const [protocolSet,  setProtocolSet]  = useState<Set<string>>(new Set(ALL_PATIENTS.filter(p => p.hasProtocol).map(p => p.id)));
  ...
  useEffect(() => {
    setGame('synapse'); setInputSrc('bp'); setHand('right');
    setGripForce(45); setDifficulty(1); setDuration(15);
    setTremor(true); setAudioHints(true); setVisualGuides(true);
    setSavedState('idle');
  }, [selectedId]);
  ...
  const filteredSidebar = ALL_PATIENTS.filter(p =>
  ...
  const NoPatientState = () => (
    <div style={{
      ...
      <h2 style={{ fontSize: 22, fontWeight: 800, color: '#0B1E33', margin: '0 0 10px' }}>
        Select a Patient
      </h2>
      <p style={{ fontSize: 14, color: '#64748b', margin: '0 0 28px', maxWidth: 340, lineHeight: 1.7 }}>
        Choose a patient from the list on the left to configure their personalised therapy protocol and game settings.
      </p>
          ...
          return (
            <button
              ...
              style={{
                ...
                background: '#fff', border: `1.5px solid ${ac}30`,
                cursor: 'pointer', transition: 'all 0.2s ease',
                fontFamily: "'Plus Jakarta Sans',sans-serif",
              }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = ac; e.currentTarget.style.background = `${ac}0a`; }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = `${ac}30`; e.currentTarget.style.background = '#fff'; }}
            >
              ...
              <span style={{ fontSize: 12, fontWeight: 700, color: '#0B1E33' }}>{p.name.split(' ')[0]}</span>
              {protocolSet.has(p.id) && <CheckCircle2 size={11} color="#22c55e" />}
            </button>
          );
        })}
      </div>
    ...
    <div className="tp-main-grid">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
        ...
        <div className="tp-card" style={{ animation: 'tpCardPop 0.50s cubic-bezier(0.22,1,0.36,1) 0.05s both' }}>
          <div className="tp-section-title">
            <span className="icon-wrap"><Gamepad2 size={15} /></span>
            Game Selection
          </div>
            ...
            {GAMES.map(g => (
              ...
              >
                ...
                <div style={{
                  ...
                  background: game === g.value ? 'rgba(99,102,241,0.10)' : '#f1f5f9',
                  border: game === g.value ? '1.5px solid rgba(99,102,241,0.30)' : '1.5px solid #e2e8f0',
                ...
                <div style={{ flex: 1, textAlign: 'left' }}>
                  <div style={{ fontSize: 13.5, fontWeight: 700, color: '#0B1E33' }}>{g.label}</div>
        ...
        <div className="tp-card" style={{ animation: 'tpCardPop 0.50s cubic-bezier(0.22,1,0.36,1) 0.12s both' }}>
          <div className="tp-section-title">
            <span className="icon-wrap"><Settings2 size={15} /></span>
            Hardware Calibration
          </div>

          {/* Input Source */}
          <div style={{ marginBottom: 20 }}>
            <div style={{ fontSize: 12.5, fontWeight: 600, color: '#475569', marginBottom: 10 }}>Input Source</div>
          ...
          <div style={{ marginBottom: 22 }}>
            <div style={{ fontSize: 12.5, fontWeight: 600, color: '#475569', marginBottom: 10 }}>Target Hand</div>
          ...
          <div style={{ marginBottom: 18 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: '#0B1E33' }}>Grip Force Sensitivity (MVC)</div>
            ...
            />
            <p style={{ fontSize: 11.5, color: '#64748b', marginTop: 7, lineHeight: 1.6 }}>
        ...
        <div className="tp-card" style={{ animation: 'tpCardPop 0.50s cubic-bezier(0.22,1,0.36,1) 0.19s both' }}>
          <div className="tp-section-title">
            <span className="icon-wrap" style={{ background: 'rgba(99,102,241,0.10)', color: '#6366f1' }}><Brain size={15} /></span>
            Game Logic Settings
          </div>
          ...
          <div style={{ marginBottom: 22 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: '#0B1E33' }}>Speed / Difficulty</div>
          ...
          {/* Duration */}
          <div style={{ marginBottom: 22 }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: '#0B1E33', marginBottom: 10 }}>Session Duration</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <span style={{ fontSize: 13, color: '#64748b', fontWeight: 500 }}>minutes per session</span>
            </div>
          </div>

          {/* Cues */}
          <div>
            <div style={{ fontSize: 13, fontWeight: 700, color: '#0B1E33', marginBottom: 12 }}>Assistance Cues</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
        ...
        <div className="tp-card" style={{ animation: 'tpCardPop 0.50s cubic-bezier(0.22,1,0.36,1) 0.07s both', position: 'sticky', top: 20 }}>
          <div className="tp-section-title">
            <span className="icon-wrap" style={{ background: 'rgba(251,191,36,0.12)', color: '#f59e0b' }}><Zap size={15} /></span>
            Live Preview
          </div>
          ...
          {/* Protocol Summary */}
          <div style={{ marginTop: 16, padding: '14px 16px', background: 'rgba(240,244,248,0.8)', borderRadius: 14, border: '1px solid rgba(226,232,240,0.8)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '7px 14px' }}>
              {[
                ...
                <div key={row.label}>
                  ...
                  <div style={{ fontSize: 12.5, fontWeight: 700, color: '#0B1E33', marginTop: 1 }}>{row.val}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="tp-btn-row" style={{ display: 'flex', gap: 9, marginTop: 14 }}>
            <button className="tp-btn-preview">
              <Play size={15} fill="currentColor" style={{ position: 'relative', zIndex: 2 }} />
        ...
        <div className="tp-card" style={{ animation: 'tpCardPop 0.50s cubic-bezier(0.22,1,0.36,1) 0.17s both' }}>
          <div className="tp-section-title">
            <span className="icon-wrap" style={{ background: 'rgba(99,102,241,0.10)', color: '#6366f1' }}><BookOpen size={15} /></span>
            ...
            {SAVED_PROTOCOLS.map((proto, i) => (
              <div key={proto.name} className="tp-protocol-item" style={{ animationDelay: `${0.22 + i * 0.07}s` }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                  <span style={{ fontSize: 13, fontWeight: 700, color: '#0B1E33' }}>{proto.name}</span>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <span style={{ fontSize: 11.5, color: '#64748b', fontWeight: 500 }}>{proto.game}</span>
                  <span style={{ fontSize: 10, color: '#94a3b8' }}>•</span>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                    <Users size={10} color="#94a3b8" />
                    <span className="mono" style={{ fontSize: 10, color: '#94a3b8', fontWeight: 600 }}>{proto.patients} patients</span>
                  </div>
                </div>
              </div>
            ))}
          ...
          <button
            style={{
              ...
              border: '1.5px dashed rgba(45,212,191,0.35)', background: 'rgba(240,253,250,0.6)',
    ...
    <div className="tp" style={{ minHeight: '100vh', background: '#F0F4F8', paddingBottom: 52 }}>
      ...
      <div className="tp-outer" style={{ maxWidth: 1340, margin: '0 auto', padding: '28px 24px', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ marginBottom: 24, animation: 'tpFadeUp 0.50s ease both', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 14 }}>
          <div>
            <p className="mono" style={{ fontSize: 9, color: 'rgba(45,212,191,0.72)', textTransform: 'uppercase', letterSpacing: '0.22em', marginBottom: 5, fontWeight: 600 }}>
            ...
            <p style={{ fontSize: 13.5, color: '#64748b', marginTop: 5, fontWeight: 500 }}>
              {patient
                ? <>Configuring protocol for <span style={{ color: '#6366f1', fontWeight: 800 }}>{patient.name}</span> · {patient.condition}</>
        ...
        <div className="tp-outer-layout">
          ...
          {/* ════════════════════════════════════════════
              PATIENT SIDEBAR
          ════════════════════════════════════════════ */}
          <div className="tp-sidebar-wrap" style={{
            ...
            <div style={{
              ...
              background: 'linear-gradient(135deg,#f8f7ff,#f0effe)',
              flexShrink: 0,
            }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 12 }}>
                <div style={{ width: 32, height: 32, borderRadius: 10, background: 'rgba(99,102,241,0.10)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#6366f1' }}>
                  <Users size={15} />
                </div>
                <div>
                  ...
                  >
                    ...
                    <div style={{ position: 'relative', flexShrink: 0 }}>
                      <div style={{
                        ...
                        background: isA ? 'linear-gradient(135deg,#6366f1,#4f46e5)' : `${ac}18`,
                      ...
                      {hasP && (
                        <div style={{ position: 'absolute', bottom: -2, right: -2, width: 12, height: 12, borderRadius: '50%', background: '#22c55e', border: '2px solid #fff', boxShadow: '0 0 4px rgba(34,197,94,0.6)' }} />
                      )}
                      {/* Status colour ring for no-protocol */}
                      {!hasP && (
                        <div style={{ position: 'absolute', bottom: -2, right: -2, width: 12, height: 12, borderRadius: '50%', background: '#fff', border: `2.5px solid ${sc}` }} />
                      )}
                    </div>
          ...
          <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', animation: 'tpFadeUp 0.50s ease 0.06s both' }}>
            ...
            {patient && (
              <div style={{
                marginBottom: 18,
                background: '#0B1E33', borderRadius: 18, padding: '16px 22px',
                ...
                <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', pointerEvents: 'none' }}>
                  <div style={{ position: 'absolute', left: 0, right: 0, height: '18%', background: 'linear-gradient(to bottom,transparent,rgba(45,212,191,0.05),transparent)', animation: 'tpScanLine 5s linear infinite' }} />
                </div>
                ...
                <div style={{ position: 'relative', zIndex: 2, display: 'flex', gap: 9, flexWrap: 'wrap' }}>
                  {patient.sub === 'AI Companion'
                    ? <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(45,212,191,0.10)', border: '1px solid rgba(45,212,191,0.22)', borderRadius: 11, padding: '7px 12px' }}>
                        <Bot size={12} color="#2DD4BF" />
                        <span style={{ fontSize: 11, fontWeight: 700, color: '#2DD4BF' }}>AI Companion</span>
                      </div>
                    : <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.09)', borderRadius: 11, padding: '7px 12px' }}>
                        <Shield size={12} color="rgba(255,255,255,0.35)" />
                        <span style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.40)' }}>Standard</span>
                      </div>
                  }
                  {protocolSet.has(patient.id) && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(34,197,94,0.10)', border: '1px solid rgba(34,197,94,0.25)', borderRadius: 11, padding: '7px 12px' }}>
                      ...
                      <span style={{ fontSize: 11, fontWeight: 700, color: '#22c55e' }}>Protocol Active</span>
                    </div>
                  )}
                </div>
              </div>
...
export default function TherapyProtocolsPage() {
  return (
    <Suspense fallback={<div style={{ minHeight: '100vh', background: '#F0F4F8', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ fontSize: 14, color: '#64748b' }}>Loading…</div></div>}>
