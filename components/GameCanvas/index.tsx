...
import { getActiveProtocol, addXpToPatient, updateHardwareStatus } from "../../app/lib/db/users";
...
const GameCanvas: React.FC = () => {
    ...
    const handleSaveAndExit = async () => {
        try {
            ...
            console.log(`✅ Session saved — uid:${uid} | protocol:${protocolId} | level:${level} | duration:${durationSeconds}s`);
            console.log(`   accuracy:${accuracy}% | peakForce:${peakForce} | endurance:${enduranceDrop}% | avgReaction:${avgReactionTime}ms`);
            ...
            {uiState === 'MENU' && (
                <div style={{position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(2,8,20,0.62)',backdropFilter:'blur(3px)',zIndex:20}}>
                    <div style={{animation:'menuin 0.48s cubic-bezier(0.22,1,0.36,1) both',background:'rgba(4,12,28,0.94)',border:'1.5px solid rgba(45,212,191,0.30)',borderRadius:26,padding:'38px 42px',maxWidth:490,width:'90%',textAlign:'center',backdropFilter:'blur(22px)',boxShadow:'0 0 70px rgba(45,212,191,0.10),0 32px 80px rgba(0,0,0,0.60)',position:'relative',overflow:'hidden'}}>
                        ...
                        <p style={{color:'rgba(255,255,255,0.28)',fontSize:10,letterSpacing:'0.25em',textTransform:'uppercase',margin:'0 0 26px'}}>Protocol A · Motor &amp; Cognitive</p>
                        ...
                        <button
                            ...
                            style={{position:'relative',overflow:'hidden',background:isConnected?'#2DD4BF':'rgba(45,212,191,0.25)',color:isConnected?'#061422':'rgba(6,20,34,0.50)',border:'none',borderRadius:999,padding:'15px 50px',fontSize:17,fontWeight:900,letterSpacing:'0.07em',cursor:isConnected?'pointer':'not-allowed',boxShadow:isConnected?'0 0 38px rgba(45,212,191,0.50)':'none',display:'inline-flex',alignItems:'center',gap:11,transition:'transform .14s'}}
                            onMouseEnter={e=>{ if(isConnected) e.currentTarget.style.transform='scale(1.05)'; }}
                            onMouseLeave={e=>(e.currentTarget.style.transform='scale(1)')}
                        >
                            {isConnected && <div style={{position:'absolute',top:0,left:0,width:'38%',height:'100%',background:'rgba(255,255,255,0.32)',animation:'shimmer 1.9s ease-in-out infinite',pointerEvents:'none'}}/>}
                            <Play size={20} fill={isConnected?'#061422':'rgba(6,20,34,0.50)'} style={{position:'relative',zIndex:1}}/>
                            <span style={{position:'relative',zIndex:1}}>{isConnected ? 'START MISSION' : 'CONNECT DEVICE FIRST'}</span>
                        </button>
                    </div>
                </div>
            )}
            ...
            {uiState === 'PLAYING' && (<>
                <div style={{position:'absolute',top:14,left:'50%',transform:'translateX(-50%)',display:'flex',gap:10,zIndex:30,pointerEvents:'none',animation:'hudin 0.38s ease both'}}>
                    <div style={{background:'rgba(4,12,28,0.74)',backdropFilter:'blur(16px)',border:'1px solid rgba(45,212,191,0.22)',borderRadius:999,padding:'7px 20px',display:'flex',flexDirection:'column',alignItems:'center',boxShadow:'0 4px 22px rgba(0,0,0,0.38)'}}>
                        ...
                        <span style={{fontSize:24,fontFamily:'monospace',fontWeight:900,color:'#fff',lineHeight:1.1}}>{score.toString().padStart(4,'0')}</span>
                    </div>
