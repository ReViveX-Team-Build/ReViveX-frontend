...
const STYLES = `
  ...
  .dtb-dropdown { position:absolute; top:calc(100% + 10px); right:0; width:340px; z-index:600; background:linear-gradient(160deg,#0d2442,#0B1E33); border:1px solid rgba(255,255,255,0.09); border-radius:20px; box-shadow:0 20px 60px rgba(0,0,0,0.45), 0 0 0 1px rgba(45,212,191,0.06); animation:notifDrop 0.22s cubic-bezier(0.22,1,0.36,1) both; overflow:hidden; }
  ...
  /* New Action Buttons for Notifications */
...
export default function DoctorTopbar() {
  ...
  // 2. LIVE LISTENER: Pending Patient Requests
  useEffect(() => {
    if (!user) return;
    const q = query(
      collection(db, "users"), 
      ...
      where("connectionStatus", "==", "pending")
    );
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
          ...
          <div ref={notifRef} style={{ position: "relative" }}>
            <button className="dtb-icon-btn" onClick={() => { setNotifOpen(p => !p); setProfileOpen(false); }}>
              <Bell size={18} color={pendingRequests.length > 0 ? "#2DD4BF" : "rgba(255,255,255,0.55)"} />
            ...
            {notifOpen && (
              <div className="dtb-dropdown">
                <div style={{ padding: "16px 16px 12px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div>
                    <div style={{ fontSize: 14, fontWeight: 700, color: "#fff" }}>Action Required</div>
                ...
                <div style={{ maxHeight: 320, overflowY: "auto" }}>
                  {pendingRequests.length === 0 ? (
                    ...
                    pendingRequests.map((req, i) => (
                      <div key={req.id} className="dtb-notif-item" style={{ animationDelay: `${i * 0.06}s` }}>
