...
import {
  ...
  Users,
  CalendarDays,
  FileText,    // Icon for Protocols
  BarChart2,   // Icon for Reports
  Bot,         // Icon for AI
  Settings,
  HelpCircle,
  LogOut,
...
const DoctorSidebar = () => {
  ...
  // These links cover the core functionality required for the doctor's workflow.
  const navItems = [
  ...
  // Secondary utilities moved to the bottom to separate them from daily tasks.
  const bottomItems = [
  ...
  return (
    // ASIDE CONTAINER
    // Fixed position to the left (left-0 top-0) with a high Z-Index (z-[100])
    // to ensure it stays visible above other content. 
    // Uses the dark navy theme color (#0B1E33).
    <aside className="h-screen w-72 bg-[#0B1E33] text-white flex flex-col fixed left-0 top-0 border-r border-white/10 shadow-2xl z-[100]">
      ...
      {/* --- HEADER SECTION --- */}
      {/* Contains the App Logo and Title */}
      <div className="p-6 mb-2 flex items-center gap-3">
        {/* Logo Container with Neon Teal Glow */}
      ...
      {/* This area expands (flex-1) to take up available space and scrolls if needed */}
          ...
          return (
            <Link key={item.href} href={item.href}>
              <div className={`
                ...
                ${isActive 
                  // Active State: Teal background, Dark text, subtle shadow
                  ? "bg-[#2DD4BF] text-[#0B1E33] font-bold shadow-lg shadow-[#2DD4BF]/20" 
                  // Inactive State: Gray text, slight white hover effect
      ...
      {/* --- BOTTOM SECTION --- */}
      {/* Contains Settings, Help, and the Logout Button */}
      {/* Background is slightly darker (#081626) to visually separate it */}
      <div className="p-4 border-t border-white/10 bg-[#081626]">
        ...
        {/* Sign Out Button */}
        {/* Styled with red accents to indicate a destructive/exit action */}
        <Link href="/">
